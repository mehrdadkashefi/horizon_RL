...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor_mean = layer_init(nn.Linear(args.num_units, envs.single_action_space._shape[0]), std=0.01)
        self.critic = layer_init(nn.Linear(args.num_units, 1), std=1)
    ...
    def get_action_and_value(self, x, rnn_state, done, action=None):
        ...
        if action is None:
            action = probs.sample()
        return action, probs.log_prob(action).sum(1), probs.entropy().sum(1), self.critic(hidden), rnn_state
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        initial_rnn_state = (next_rnn_state[0].clone(), )
